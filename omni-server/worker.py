...
def create_app(engine):
    ...
    def health():
        return dict(ready=True, busy=lock.locked(), model='MiniCPM-o-4.5-AWQ',
                    ...
                    mode='vad_segmented_end_to_end', **stats)

    @app.post('/generate')
    async def generate(request: Request):
        body = bytearray()
        async for data in request.stream():
            if len(body) + len(data) > 3_000_000:
...
class MiniCPMEngine:
    def __init__(self, model_path, *, male_reference=None, female_reference=None):
        import torch
        ...
        self.model = AutoModel.from_pretrained(str(model_path), trust_remote_code=True,
            ...
            init_vision=False, init_audio=True, init_tts=True, local_files_only=True)
        ...
        with self.torch.inference_mode():
            ...
            for message in messages:
                ...
                if message['role'] == 'user':
                    ...
                    content = '<|im_end|>\n<|im_start|>assistant\n' + content + '<|im_end|>\n'
                chunks = [content] if isinstance(content, str) else [content[i:i+16000] for i in range(0,len(content),16000)]
