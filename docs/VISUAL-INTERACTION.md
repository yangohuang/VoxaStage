...
已完成真实视觉模型的受控探针、32次正式输入对照及三种数字人的9轮浏览器流程，详见[实测报告](REAL-MULTIMODAL-BASELINE.md)。仅语音缺图时编造视觉属性的问题已复现，不能据此宣称整体语义质量通过。现有默认语音服务没有启用此功能。图片理解来自 MiniCPM-o，本项目贡献是能力探测、画面与轮次绑定、有界历史及交互清理，不是新的视觉模型。
...
模型环境和固定版本见 [MiniCPM 部署](../omni-server/README.md)。本次已实际加载视觉编码器；报告记录整卡显存占用，包含同时驻留的ASR服务，尚不能当作单模型显存增量或峰值。
...
等待“画面已确认”后输入问题或说话。附件只绑定下一条文字或下一轮 VAD 完成的语音，后来的图片不会改变已启动的请求。回答后可以提交第二张图并询问前后变化；最多保留两张最近画面。移除按钮只移除尚未绑定的附件，不删除已经进入历史的图片。断开会话停止摄像头、清除尚未绑定的画面并保存有界历史；载入同一对话可继续，删除对话才移除持久记录。迟到的文件解码或摄像头授权结果不能进入新会话。
...
旧会话、重复/倒退序号、坏图片与越界值明确拒绝，拒绝不替换先前已确认的附件。新输入得到递增 `turn_id`；模型历史仍遵循六轮、60秒语音及12000字符限制。第三张图进入时先移除最早图像数据，保留对应问答，并注明该轮图片已经不可见。回答只保留确认播放的完整语句段，未确认部分写入中断标记；生成文字可在会话记录中查看，但不等同于模型的已听上下文。
...
本地测试覆盖附图与原PCM绑定、裁剪、关闭期间异步绑定、晚到结果、摄像头授权和解码后的资源清理。模型响应替身用于检查会话协议，不用于识图准确率结论。本次受控测试已覆盖真实固定模型的图文、图音、两帧变化及四种输入条件；原始输入哈希、逐次输出和失败已保留。更广泛场景、真实麦克风和动态口型质量仍需另行验收。

路线及各项验收见[实施计划](plans/2026-09-19-visual-interaction.md)。人工口型质量、物理设备声学标定和后续针对性方法实验仍为独立待办。

[四组输入对照工具](MULTIMODAL-EVALUATION.md)已提供仅语音、图片描述、图文、图音的串行评测入口，绑定输入与请求摘要，保留失败和部分输出。工具测试通过不代表模型实验完成；真实语义判读仍为空。
...
[机器记录](benchmarks/2026-09-19-visual-ui.json)保留实际图片来源/时间/哈希、三轮模型请求结构、源码摘要和失败记录。虚拟摄像头及模拟回复不验证真实摄像头采集质量、模型识图、口型或端到端延迟。该本地检查点之后的真实模型结果见[视觉与语音实测](REAL-MULTIMODAL-BASELINE.md)。上述检查点记录的是 2026-09-19 状态。2026-09-20 的日常演示已启用视觉，界面仍依据 worker 健康接口决定是否显示画面输入；见[当前状态](STATUS.md)。
