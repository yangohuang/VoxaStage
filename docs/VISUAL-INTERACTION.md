...
[机器记录](benchmarks/2026-09-19-visual-ui.json)保留实际图片来源/时间/哈希、三轮模型请求结构、源码摘要和失败记录。虚拟摄像头及模拟回复不验证真实摄像头采集质量、模型识图、口型或端到端延迟。该本地检查点之后的真实模型结果见[视觉与语音实测](REAL-MULTIMODAL-BASELINE.md)。默认演示服务仍未开启视觉。
