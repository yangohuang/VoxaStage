...
每段回复使用独立 transaction/session 和 WebSocket。打断发 is_interrupt + is_end 并关闭连接；关闭连接是本地传输清理，不代表上游提供 GPU 清理确认。已通过取消后再次实际生成验证恢复。若 DINet 没有可用 worker，连接错误会显示为会话错误；不会替换成其他模型冒充成功。
